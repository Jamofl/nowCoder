...
public class Q4 {
    ...
    public static int cal(int n, int day, int range, int target, int[] laozhi, int[] position){
        int trySum = 0;
        ...
        Map<Integer, Integer> map = new HashMap<>(); // 存了每个位置对应的自己 + 左右range位置上的laozhi之和
        Set<Integer> alreadySit = new HashSet<>();
        for (int i = 0; i < n; i ++)
            map.put(i, 0);

        for (int i = 0; i < day; i ++){
            int curLaozhi = laozhi[i];
            int curPos = position[i];
            alreadySit.add(curPos);
            for (int p = curPos - range; p <= curPos + range; p ++){
                if (p < 0 || p >= n)
                    continue;
                map.put(p, map.get(p) + curLaozhi);
